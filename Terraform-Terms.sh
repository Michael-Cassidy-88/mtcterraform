...
# conditional that outputs the value of 3 if the variables are the same.  If they are not, the output will be 1

terraform providers schema -json | jq '.' > schema.json
# print detailed schemas for the providers used in the current configuration, outputs in json format, pipes to jq, and sends it to a file

substr(1234, 0, 4)
# returns a portion of the string starting with the 0 index and includes 4 objects from the string following that index

uuid()
# returns a universally unique id as a string

substr(uuid(), 0, 3)
# returns a portion of the uuid starting with the 0 index, and 3 objects in length

terraform output -json | jq '."instances"."value"'
# outputs instances values

kubectl create -f nginx-dep.yaml 
# deploys K8s resources

split("=", "export KUBECONFIG=../k3s-mtc_node-36758.yaml")[1]
# splits the value of the string at the "=" and returns the second part 